...
// Endpoint: Imágenes RAW de WhatsApp (JOHN)
app.get('/api/raw-imagenes', async (req, res) => {
  try {
    const instancia = req.query.instancia || 'JOHN';
    const { rows } = await pool.query(
      `SELECT 
         id, hash_largo, hash_corto, grupo_raw, usuario_raw, nombre_push, 
         caption, url_imagen, COALESCE(conteo, 1) AS conteo, estado, instancia, created_at, timestamp_msg
       FROM registros_raw 
       WHERE instancia = $1 AND url_imagen IS NOT NULL AND url_imagen != ''
